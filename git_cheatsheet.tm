...
<\body>
  ...
  <subsection|<section|<with|font-shape|small-caps|The Index, Commit and
  Workspace>>>

  Every git repository has a special storage area, known as the index. When
  you <with|font-family|tt|git add> something, it copies the contents of the
  specified files into the index. So, if you change the file again after
  <with|font-family|tt|git add>, and then commit, the new changes won't be
  included.
  ...
  commit. <with|font-family|tt|git add> is used when you've modified a file,
  or created a new file. In either case, it updates the index with a new file
  version.
  ...
  <\equation*>
    <matrix|<tformat|<table|<row|<cell|<text|Commit>>>>>> \<leftrightarrow\>
    <matrix|<tformat|<table|<row|<cell|<text|Index>>>>>> \<leftrightarrow\>
    <matrix|<tformat|<table|<row|<cell|<text|Workspace>>>>>>
  </equation*>

  <\itemize>
    <item><with|font-shape|small-caps|>Ways to Spell
    <with|font-family|tt|commit>
  ...
  <section|<with|font-shape|small-caps|Rebase>>
  ...
  <section|<with|font-shape|small-caps|Basic Data Model>>
</body>
